...
//        defaultDate: +2,
        ...
        adl_flight = $("#flight_adl_val").val();  
        child_flight = $("#flight_chld_val").val();  
        infant_flight = $("#flight_infant_val").val();   
       
        $("#flight_adl").html(adl_flight);
        $("#flight_chld").html(child_flight);
        $("#flight_infant").html(infant_flight);
     
    //$("#flight_main_to_optional").hide();   
   
    $("#flight_main_to_optional").click(function()
    { 
        //$("#flight_main_to").attr("placeholder", 'Date of Return (Optional)'); 
        $("#flight_main_to").val(' ');
        $("#FlightJourneyType").val(1);
        //////////$("#modify_return_optional").css("background-color", "#d6d7d8");
        $("#flight_main_to_optional").hide();     
    });
...
*/
    ...
    $("body").click(function(e)
    {
        ...
        {   
          if(sorce_set==1 && source_first_name!='')
            {
        ...
        { 
           if(dest_set==1 && source_first_name!='')
            { 
    ...
    {     
        $("#search-box-flight").val('');    
        $("#flight_source_id").val('');
    ...
    $("#search-box1-flight").click(function()
    {     
    ...
    $("#search-box-flight").keyup(function(){ 
        ...
        success: function(data){  
            ...
            /*var image = new Image();
            var path=  URL+'/images/country_flag/'+obj[0].country_code+'.jpg';
            image.src = path;
            ...
            for(i=0;i<obj.length;i++)
            {
               if (!image.complete)
              {
                  source_list +='<li onClick=selectFlightSource("'+encodeURIComponent(obj[i].source_name)+'","'+obj[i].source_id+'","'+obj[i].airport_code+'")>'+obj[i].source_name+' ('+obj[i].source_id+')['+obj[i].airport_code+'],'+obj[i].airport_name+','+obj[i].country_code+'<div class=mapIcon><i class="fa fa-plane searchIcon" aria-hidden="true"></i></div>&nbsp;&nbsp;&nbsp;</li>';
              }
              else
              {
               source_list +='<li onClick=selectFlightSource("'+encodeURIComponent(obj[i].source_name)+'","'+obj[i].source_id+'","'+obj[i].airport_code+'")>'+obj[i].source_name+' ('+obj[i].source_id+')['+obj[i].airport_code+'],'+obj[i].airport_name+','+obj[i].country_code+'<div class=mapIcon><i class="fa fa-plane searchIcon" aria-hidden="true"></i></div>&nbsp;&nbsp;&nbsp;<img src="'+URL+'/images/country_flag/'+obj[0].country_code+'.jpg" alt="img"/></li>';
              }
            }*/
        ...
        success: function(data){
            ...
            {  
                ...
                for(i=0;i<obj.length;i++)
                {
                    if (!image1.complete)
                   {
                     dest_list +='<li onClick=selectFlightDest("'+encodeURIComponent(obj[i].source_name)+'","'+obj[i].source_id+'","'+obj[i].airport_code+'")>'+obj[i].source_name+' ('+obj[i].source_id+')['+obj[i].airport_code+'],'+obj[i].airport_name+','+obj[i].country_code+'<div class=mapIcon><i class="fa fa-plane searchIcon" aria-hidden="true"></i></div>&nbsp;&nbsp;&nbsp;</li>';
                   }
                   else
                   {
                    dest_list +='<li onClick=selectFlightDest("'+encodeURIComponent(obj[i].source_name)+'","'+obj[i].source_id+'","'+obj[i].airport_code+'")>'+obj[i].source_name+' ('+obj[i].source_id+')['+obj[i].airport_code+'],'+obj[i].airport_name+','+obj[i].country_code+'<div class=mapIcon><i class="fa fa-plane searchIcon" aria-hidden="true"></i></div>&nbsp;&nbsp;&nbsp;<img src="'+URL+'/images/country_flag/'+obj[0].country_code+'.jpg" alt="img"/></li>';
                   }
                }*/
...
{       
    CT = parseInt(window.adl_flight) + parseInt(window.child_flight) + parseInt(window.infant_flight);
    if( CT > 8)
    {
        //alert("xx1");
    ...
    }else
    {
        ...
        $("#traveler_details").val(CT+ ' Traveller(s) '+str);
        $("#flightSearch").hide();
    }
}  
...
function inc_members(val)
{
    ...
    adl_flight1 = parseInt(window.adl_flight);
    child_flight1 = parseInt(window.child_flight);
    infant_flight1 = parseInt(window.infant_flight);
    ...
    { 
        ...
        {     
            CtCheck = paCheck(1); 
            if( adl_flight1 < CtCheck)
            {    
                if(adl_flight1 < 8)
                {
                    adl_flight1 +=1; 
                    window.adl_flight = adl_flight1;
                    $("#flight_adl").html(adl_flight1); 
                    $("#flight_adl_val").val(adl_flight1);  
                }
            }
            /*else
        ...
        if(val==2)
        {
            ...
            CtCheck = paCheck(2); 
            if( child_flight1 < CtCheck)
            {
                if( child_flight1 < 8)
                {
                    child_flight1 +=1;     
                    window.child_flight = child_flight1;
                    $("#flight_chld").html(child_flight1); 
                    $("#flight_chld_val").val(child_flight1);             
                }
            }
            /*else
        ...
        {
            CtCheck = paCheck(3); 
            if( infant_flight1 < CtCheck)
            {
                if( infant_flight1 < 8)
                {
                    infant_flight1 +=1;   
                    window.infant_flight = infant_flight1;
                    $("#flight_infant").html(infant_flight1);  
                    $("#flight_infant_val").val(infant_flight1); 
                }
            }
            /*else
