...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (airlineName) {
    // Set the title to the airline name
    document.getElementById("airline-title").textContent = airlineName;

    // Update the image source based on the airline name (you can replace this with your image URLs)
    const imageSrc = getImageSource(airlineName);
    ...
    document.getElementById(
      "airline-image-1"
    ).innerHTML = `<img src="${imageSrc2}" alt="${airlineName}" height="40px" style="padding:2px;">`;

    const aboutUsDescription = getAboutUsDescription(airlineName);
    document.getElementById("about-us-description").textContent =
      aboutUsDescription;

    // You can also update other content here if needed
  }
});

// Function to get the image source based on the airline name
...
function getAboutUsDescription(airlineName) {
  // Retrieve the description from the airline data
